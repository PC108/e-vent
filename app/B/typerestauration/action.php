...
$table = 't_typerestauration_tyres';
...
// Vérification des champs obligatoires, plus le mail, plus les années
    if (($_POST["TYRES_nom_fr"] == "") || ($_POST["TYRES_nom_en"] == "") || ($_POST["TYRES_montant_defaut"] == "") || ($_POST["TYRES_capacite_defaut"] == "")) {
	$checkRequired = "bad";
    } else {
	// $formatData['TYRES_nom_fr'] = 'LOWERFIRST';
	// $formatData['TYRES_nom_en'] = 'LOWERFIRST';
	$formatData = array();
...
if ($checkRequired == "bad") {
    ...
    switch ($action) {
	case "maj":
	    $query = adn_creerUpdate($table, 'TYRES_id', $id, array('action', 'TYRES_id', 'Submit', 'pageNum'), $autresData, $formatData, $checkArrayVide);
	    adn_mysql_query($query, $connexion, array('maj_ok', 'maj_ko'), array('message_user', 'message_debug'));
	    break;
	case "add":
	    $query = adn_creerInsert($table, array('action', 'TYRES_id', 'Submit', 'pageNum'), $autresData, $formatData);
	    adn_mysql_query($query, $connexion, array('add_ok', 'add_ko'), array('message_user', 'message_debug'));
	    break;
	case "del":
	    if ($autoriseDelete) {
		$query = "DELETE FROM " . $table . " WHERE TYRES_id='" . mysql_real_escape_string($id) . "'";
