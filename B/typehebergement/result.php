...
/*                       Titre                             */
/*	* ******************************************************** */
$titre	=	"Back-Office "	.	$_SESSION['info_client']['nom']	.	" : "	.	$menuInfos['EVENEMENTS']['L3']['nom']	.	"  - Résultat de la recherche";
									...
									<?php	}	else	{	?>
												<table class="table_result" cellspacing="0">
															...
															<tr>
																		...
																		<th>Montant <img class="bt_help pointer" src="../_media/bo_help_16.png" width="16" height="16" align="absmiddle" alt="aide"/></th>
																		<th nowrap>Capacité totale de places <img class="bt_help pointer" src="../_media/bo_help_16.png" width="16" height="16" align="absmiddle" alt="aide"/></th>
															...
															<?php	if	($NbreRows_RS	>	0)	{	?>
																		<?php	while	($row	=	mysql_fetch_object($RS))	{	?>
																					<tr class="to_highlight" valign="top">
																								<?php	if	($autoriseMaj)	{	?><td align="center"><a href="add.php?id=<?php	echo	$row->TYHEB_id	?>&pageNum=<?php	echo	$pageNum	?>"><img src="../_media/bo_edit.png" width="16" height="16" border="0" alt=""/></a></td><?php	}	?>
																								<?php	if	($autoriseComment)	{	?> <td align="center" bgcolor="#EEEEEE">
																														<div class="js_commentaire pointer" id="<?php	echo	$row->FK_CMTTYHEB_id	?>" idfiche="<?php	echo	$row->TYHEB_id	?>">
																																	<?php	if	(is_null($row->FK_CMTTYHEB_id))	{	?><a href="../commentaire/add.php?table=TYHEB&idfiche=<?php	echo	$row->TYHEB_id	?>&pageNum=<?php	echo	$pageNum	?>">[+]</a>
																																	<?php	}	else	{	?><a href="../commentaire/add.php?table=TYHEB&idcom=<?php	echo	$row->FK_CMTTYHEB_id	?>&idfiche=<?php	echo	$row->TYHEB_id	?>&pageNum=<?php	echo	$pageNum	?>"><img src="../_media/bo_comment.gif" width="18" height="21" border="0" alt=""/></a>
																																	<?php	}	?>
																														</div>
																											</td>
																								<?php	}	?>
																								<td class="note"><i><?php	echo	$row->TYHEB_ordre;	?></i></td>
																								<td nowrap><?php	echo	$row->TYHEB_nom_fr;	?></td>
																								<td nowrap><?php	echo	$row->TYHEB_nom_en;	?></td>
																								<td nowrap><?php	echo	$row->TYHEB_montant_defaut;	?></td>
																								<td nowrap><?php	echo	$row->TYHEB_capacite_defaut;	?></td>
																								<td>
																											<?php
																											if	(!is_null($row->TYHEB_description_fr))	{
																														$str_Description	.=	'<div id="fr_'	.	$row->TYHEB_id	.	'" class="invisible">'	.	nl2br($row->TYHEB_description_fr)	.	'</div>';
																														echo	('<div class="js_description pointer" id="fr_'	.	$row->TYHEB_id	.	'">'	.	adn_cropTexte($row->TYHEB_description_fr,	10)	.	'</div>');
																											}
																											?>
																								</td>
																								<td>
																											<?php
																											if	(!is_null($row->TYHEB_description_en))	{
																														$str_Description	.=	'<div id="en_'	.	$row->TYHEB_id	.	'" class="invisible">'	.	nl2br($row->TYHEB_description_en)	.	'</div>';
																														echo	('<div class="js_description pointer" id="en_'	.	$row->TYHEB_id	.	'">'	.	adn_cropTexte($row->TYHEB_description_en,	10)	.	'</div>');
																											}
																											?>
																								</td>
																								<?php	if	($autoriseDelete)	{	?><td align="center"><img class="bt_delete pointer" src="../_media/bo_delete.png" id_todelete="<?php	echo	$row->TYHEB_id;	?>" width="16" height="16" border="0"alt=""/></td><?php	}	?>
																					</tr>
																		<?php	}	}	?>
												...
												<div id="help" title='<img src="../_media/bo_help_32.png" align="absmiddle"/> aide'>
															...
															<p>Montant: <span class="note">Cette valeur correspond au montant d'un type d'hébergement <b>par défaut</b>.</span></p>
															<p>Capacité totale de places: <span class="note">Cette valeur correspond à la capacité <b>par défaut</b> pour un type d'hébergement.
																					<br/>Ces valeurs apparaitront lorsque vous créerez un événement et pourront être personnalisées à ce moment là, si nécessaire.</span></p>
												</div>
									<?php	}	?>
