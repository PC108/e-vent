...
/*                       Titre                             */
/*	* ******************************************************** */
$titre	=	"Back-Office "	.	$_SESSION['info_client']['nom']	.	" : "	.	$menuInfos['COMMANDES']['L3']['nom']	.	"  - Résultat de la recherche";
						...
						<div id="contenu">
									...
									<?php	}	else	{	?>
															...
															<?php	if	($NbreRows_RS	>	0)	{	?>
																		<?php	while	($row	=	mysql_fetch_object($RS))	{	?>
																					<tr class="to_highlight" valign="top">
																								<td align="center"><?php	if	($autoriseMaj	&&	$row->TYDON_id	>	1)	{	?><a href="add.php?id=<?php	echo	$row->TYDON_id	?>&pageNum=<?php	echo	$pageNum	?>"><img src="../_media/bo_edit.png" width="16" height="16" border="0" alt=""/></a><?php	}	?></td>
																								<?php	if	($autoriseComment)	{	?> <td align="center" bgcolor="#EEEEEE">
																														<div class="js_commentaire pointer" id="<?php	echo	$row->FK_CMTTYDON_id	?>" idfiche="<?php	echo	$row->TYDON_id	?>">
																																	<?php	if	(is_null($row->FK_CMTTYDON_id))	{	?>
																																				<a href="../commentaire/add.php?table=TYDON&idfiche=<?php	echo	$row->TYDON_id	?>&pageNum=<?php	echo	$pageNum	?>">[+]</a>
																																	<?php	}	else	{	?>
																								...
																								<?php	}	?>
																								<td class="note"><i><?php	echo	$row->TYDON_ordre;	?></i></td>
																								<td nowrap><?php	echo	$row->TYDON_nom_fr;	?></td>
																								<td nowrap><?php	echo	$row->TYDON_nom_en;	?></td>
																								<td><?php	if	($row->TYDON_visible)	{	?><img src="../_media/bo_visible.png" width="16" height="16" border="0" alt="visible"/><?php	}	else	{	?><img src="../_media/bo_invisible.png" width="16" height="16" border="0" alt="invisible"/><?php	}	?></td>
																								<td align="center"><?php	if	($autoriseDelete	&&	$row->TYDON_id	>	1)	{	?><img class="bt_delete pointer" src="../_media/bo_delete.png" id_todelete="<?php	echo	$row->TYDON_id;	?>" width="16" height="16" border="0"alt=""/><?php	}	?></td>
																					</tr>
																		<?php	}	}	?>
															<tr>
												...
												<div id="help" title='<img src="../_media/bo_help_32.png" align="absmiddle"/> aide'>
															...
															<p>Visible : <span class="note">Si oui, ce type de don sera visible et séléctionnable pour l'utilisateur.
																		<br />Pour désactiver un type de don déjà créé, passez-le en invisible.</p>
												</div>
									<?php	}	?>
